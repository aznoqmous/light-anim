...
export class LightContainer{
  ...
  getInnerContent(){
      this.innerContents = this.el.getElementsByClassName('light-content');
      if(!this.innerContents.length) {
          ...
          this.innerContent = this.innerContents[0];
      }
  }
  getConfig( config ){
  ...
  initImgContainer(){
      ...
      if( this.type.stretchLeft  ){
        this.stretchLeft();
        this.bindStretchLeftResize();
      }
      if( this.type.stretchRight ){
        this.stretchRight();
        this.bindStretchRightResize();
      }
  }
  getElementOffsetLeft(el){
    var offsetX = el.getBoundingClientRect().left;
    return offsetX;
  }
  ...
  getContainerOffsetRight(){
      var offsetX = window.innerWidth - this.el.getBoundingClientRect().right;
      return offsetX;
  }
  ...
  displayParams(){
      var paramsEl = document.createElement('em');
      ...
      if(this.innerContent) {
          if( this.type.fullWidth ) {
              for (var i = 0; i < this.innerContents.length; i++) {
                  var innerContent = this.innerContents[i];
                  var obj = {
                      offsetLeft: this.getElementOffsetLeft(innerContent),
                      offsetTop: innerContent.offsetTop,
              ...
              this.createDataObject(innerContent);
          }
      }
  ...
  placeObject(img){
      ...
      while(1){
          ...
          var newPosX = Math.random() * ( this.wrapper.offsetWidth - newWidth ) ;
          var newPosY = Math.random() * ( this.wrapper.offsetHeight - newHeight ) ;
  ...
  bindFullWidthResize(){
      ...
      window.addEventListener('resize', function(){ self.stretchWidth(); });
  }
  bindStretchLeftResize(){
    var self = this;
    window.addEventListener('resize', function(){ self.stretchLeft() });
  }
  bindStretchRightResize(){
    var self = this;
    window.addEventListener('resize', function(){ self.stretchRight() });
  }

  stretchLeft(){
    var offsetLeft = this.getContainerOffsetLeft() ;
    this.wrapper.style.left = - offsetLeft + 'px';
    this.wrapper.style.width = this.el.offsetWidth + offsetLeft + 'px';
  }
  stretchRight(){
    var offsetRight = this.getContainerOffsetRight();
    this.wrapper.style.right = - offsetRight + 'px';
    this.wrapper.style.width = this.el.offsetWidth + offsetRight + 'px';
  }
  stretchWidth(){
